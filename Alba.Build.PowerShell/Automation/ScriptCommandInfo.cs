...
{
    private readonly PSBuildTaskContext _ctx;

    public IList<ParameterMetadata> Parameters { get; } = [ ];

    public CmdletBindingAttribute? Cmdlet { get; }

    private ScriptCommandInfo(PSBuildTaskContext ctx) => _ctx = ctx;

    public ScriptCommandInfo(PSBuildTaskContext ctx, ScriptBlockAst ast) : this(ctx)
    {
    ...
    public ScriptCommandInfo(PSBuildTaskContext ctx, Command command) : this(ctx)
    {
        Attempt(() => {
    ...
    private void Attempt(Action action)
    {
        try {
            ...
            _ctx.Host.UI.LogException(LogLevel.Error, e, withStackTrace: null,
